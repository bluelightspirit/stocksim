...
public:
	...
	void set_attributes(map<string, int> given_attributes); // sets attributes of Stock (literal)
	...
	Stock(double given_price, map<string, int> given_attributes); // constructs a Stock with an empty String, the passed in price, and passed in attributes vector
	Stock(string given_symbol, double given_price, map<string, int> given_attributes, int given_max_quantity, int given_used_quantity); // constructs a Stock given a symbol, attributes, and max quantity all passed in
...
private:
	...
	map<string, int> attributes; // attributes of Stock can be interpreted as a quantity-trader map (empty map is set if not set) - non-pointer Trader gives error to access the map
